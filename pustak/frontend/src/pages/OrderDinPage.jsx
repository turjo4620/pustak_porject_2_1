...
export default function OrderDinPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('http://localhost:5000/api/categories').then(r => r.json()).catch(() => null),
      fetch('http://localhost:5000/api/authors').then(r => r.json()).catch(() => null),
      fetch('http://localhost:5000/api/publications').then(r => r.json()).catch(() => null),
    ])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/books/search?q=${encodeURIComponent(term)}&limit=24`)
